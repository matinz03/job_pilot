...
JobPilot is a full stack AI-powered job hunting assistant. The user sets up their profile once, uploads their resume, and the agent automatically discovers relevant jobs from Adzuna — scoring each one against the user's profile with the project's AI model. For jobs they're interested in, the agent researches the company across their public web pages and builds a structured dossier — company overview, tech stack, culture, why the role exists, and interview prep. The user reviews everything and applies with one click.
...
- Two options on upload:
  - "Extract from Resume" → the AI model parses the resume and auto-fills empty profile form fields, which the user reviews before saving
  - "Skip" → resume stored as-is, profile unchanged
- User can manually edit any profile field at any time
- User can generate a clean professional PDF resume from their current profile data using the AI model
...
- The AI model scores each job 0-100 against user profile
...
- Match score section:
  ...
  - Match reason paragraph from the AI model
...
- The AI model synthesizes all extracted content into structured dossier
- Dossier displayed on job details page
- If company site cannot be found — the AI model generates best dossier from company name and job description alone
...
- Resume PDF upload with optional profile auto-fill via the AI model
- Resume PDF generation from profile data using the AI model
- Adzuna API job discovery — searches by title and location, category filtered to IT jobs
- AI job matching with score, reason, matched skills, missing skills
- Job details page with full structured description
- Company Research Agent — single Browserbase session browses company public pages, the AI model builds dossier
...
- Match scores feel accurate and the reasoning makes sense
