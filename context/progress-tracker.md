...
- Auth uses `@insforge/sdk` and its SSR helpers. Next.js 16 `proxy.ts` protects authenticated routes; `middleware.ts` is deprecated. OAuth now keeps its PKCE verifier in a short-lived httpOnly cookie, exchanges the callback server-side, and refreshes cookies before both protected pages and protected API routes; API handlers retain their own user checks. The callback origin prefers `NEXT_PUBLIC_APP_URL` but derives the forwarded request origin when that optional setting is absent.
...
- Feature 14 supplies the installed `recharts` chart presentation matching `context/designs/dashboard.png`. The shared Navbar now has the dashboard, search, and profile icons shown in the reference, so every existing Navbar instance receives the same navigation treatment.
...
- Feature 17 queries PostHog directly with one server-only personal key and project ID. Three parallel HogQL aggregate queries filter `distinct_id` to the signed-in user: 30-day daily `job_found`, 30-day score-band `job_found`, and 7-day daily `company_researched`. The server returns only chart aggregates, never raw events. Each chart independently becomes `No data yet` for zero matching events or `Analytics unavailable` on a query failure, so analytics cannot block the dashboard. Each populated chart exposes a token-styled hover tooltip with its exact metric count.
- The shared Navbar now has an accessible mobile hamburger panel with the same three routes, active state, and authentication action as desktop navigation. `app/icon.tsx` crops the supplied logo mark for the generated app icon. PostHog ingest rewrites derive their region from `NEXT_PUBLIC_POSTHOG_HOST` rather than pinning Europe.
