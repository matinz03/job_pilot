...
Extract from Resume button — the AI model reads uploaded PDF and auto-fills profile form fields.
...
- The AI model reads extracted text and returns structured JSON matching all profile field names
...
Generate a clean professional PDF resume from current profile data using the AI model.

**Logic:**

- POST /api/resume/generate
- Reads current profile data from profiles table
- The AI model generates professional resume content:
  - Professional summary paragraph
  - Polished work experience bullet points
  - Clean professional language throughout
- @react-pdf/renderer renders the AI model output into clean single-page PDF using renderToBuffer()
...
- For each job returned:
  ...
  - The AI model scores job against user profile:
...
- AI Match Reasoning section — match reason paragraph from the AI model
...
- Derive company homepage URL by following the Adzuna redirect with server-side fetch() — no browser needed for this step:
  ...
  - If Stagehand gets no meaningful content (oneLiner and productSummary empty) — skip browser research entirely, proceed to AI synthesis with job description and profile only
...
- Close Browserbase session after homepage + max 3 sub-pages
  **AI synthesis (runs after browser closes):**
...
- Always return a dossier — never fail silently. If browser research failed, the AI model synthesizes from job description and profile alone.
