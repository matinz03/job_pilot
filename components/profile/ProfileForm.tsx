...
import { cloneElement, createContext, isValidElement, startTransition, useActionState, useContext, useEffect, useRef, useState, type ChangeEvent, type DragEvent, type FormEvent, type MouseEvent, type KeyboardEvent, type ReactElement, type ReactNode } from "react";
...
export function ProfileForm({ initialProfile }: ProfileFormProps) {
  ...
  function handleDropzoneClick(event: MouseEvent<HTMLDivElement>) {
    if (isPending || isExtracting) return;
    // The dropzone wraps its own buttons and the resume link. A click that lands on any of
    // them belongs to that control, not to the upload surface.
